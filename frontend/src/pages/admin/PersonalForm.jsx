...
export default function PersonalForm() {
  ...
  const [personalAtivo, setPersonalAtivo] = useState(true)

  useEffect(() => {
    if (!isEdicao) return
    personaisService.buscarPorId(id)
      .then(data => {
        setForm({ nome: data.nome, email: data.email, telefone: mascaraFone(data.telefone), senha: '' })
        setPersonalAtivo(data.ativo)
      })
  ...
  async function salvar() {
    ...
    if (!isEdicao && !form.senha) { setErro('Senha é obrigatória para novo personal'); return }
    setSalvando(true); setErro(null)
    try {
      if (isEdicao) {
        await personaisService.atualizar(id, form)
      } else {
        await personaisService.criar(form)
      }
  ...
  async function toggleAtivo() {
    const acao = personalAtivo ? 'inativar' : 'reativar'
    if (!confirm(`Tem certeza que deseja ${acao} este personal?`)) return
    setInativando(true)
    try {
      await personaisService.toggleAtivo(id)
      navigate('/admin/personais')
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 900, color: '#1A1A1A', textTransform: 'uppercase', letterSpacing: '0.03em', marginBottom: 6 }}>
            {isEdicao ? 'Editar Personal' : 'Novo Personal'}
          </h1>
          <p style={{ fontSize: 14, color: '#8A7F76' }}>
            {isEdicao ? 'Atualize os dados do personal trainer.' : 'Preencha os dados para criar a conta do personal.'}
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
          {isEdicao && (
            <BtnExcluir onClick={toggleAtivo} loading={inativando} label={personalAtivo ? 'Inativar' : 'Reativar'} />
          )}
          <BtnCancelar onClick={() => navigate('/admin/personais')} />
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E0D6CA', borderRadius: 20, padding: 32, display: 'flex', flexDirection: 'column', gap: 20, boxShadow: '0 2px 16px rgba(0,0,0,0.04)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <Campo label="Nome completo">
            <input style={inputStyle} placeholder="Ex: João Silva" value={form.nome} onChange={set('nome')} />
          </Campo>

          <Campo label="E-mail">
            <input style={inputStyle} type="email" placeholder="Ex: joao@academia.com" value={form.email} onChange={set('email')} />
          </Campo>
        </div>
        ...
        {isEdicao && (
          <div style={{ padding: '12px 16px', borderRadius: 10, background: '#F9F5F0', border: '1px solid #E0D6CA', fontSize: 13, color: '#8A7F76' }}>
            A senha provisória fará o personal redefinir no primeiro acesso.
          </div>
        )}
