...
async function enviarEmail(email) {
  ...
  await pool.request()
    ...
    .input('cod',   sql.Char(6),      codigo)
    .input('exp',   sql.DateTime2,    expiraEm)
    .query(`INSERT INTO dbo.otp_verificacao (email, codigo, expira_em) VALUES (@email, @cod, @exp)`)

  const { enviarOTPEmail } = require('./email.service')
...
async function verificarEmail(email, codigo) {
  ...
  const otp = r.recordset[0]
  if (!otp) throw Object.assign(new Error('Código inválido'), { status: 400 })
  if (otp.verificado) throw Object.assign(new Error('Código já utilizado'), { status: 400 })
  if (new Date(otp.expira_em) < new Date()) throw Object.assign(new Error('Código expirado. Solicite um novo.'), { status: 400 })
  if (otp.tentativas >= OTP_MAX_TENTATIVAS) throw Object.assign(new Error('Muitas tentativas. Solicite um novo código.'), { status: 400 })
  ...
  if (!check.recordset[0]) throw Object.assign(new Error('Código incorreto'), { status: 400 })
...
async function checarTokenEmail(email, token) {
  const pool = await getPool()
  const r = await pool.request()
    .input('email', sql.VarChar(120), email)
    .input('token', sql.VarChar(36),  token)
    .query(`
      SELECT TOP 1 id_otp_verificacao FROM dbo.otp_verificacao
      WHERE email = @email AND token = @token AND verificado = 1
        AND criado_em >= DATEADD(MINUTE, -30, SYSUTCDATETIME())
    `)
  return !!r.recordset[0]
}

module.exports = { enviar, verificar, checarToken, enviarEmail, verificarEmail, checarTokenEmail }
