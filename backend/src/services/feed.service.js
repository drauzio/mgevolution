...
async function reagir(id_feed, id_usuario) {
  const pool = await getPool()
  const jaExiste = await pool.request()
    .input('f', sql.Int, id_feed)
    .input('u', sql.Int, id_usuario)
    .query(`SELECT 1 FROM dbo.feed_reacao WHERE id_feed = @f AND id_usuario = @u`)

  if (jaExiste.recordset.length) {
    await pool.request()
      .input('f', sql.Int, id_feed)
      .input('u', sql.Int, id_usuario)
      .query(`DELETE FROM dbo.feed_reacao WHERE id_feed = @f AND id_usuario = @u`)
    return { curtiu: false }
  } else {
    await pool.request()
      .input('f', sql.Int, id_feed)
      .input('u', sql.Int, id_usuario)
      .query(`INSERT INTO dbo.feed_reacao (id_feed, id_usuario) VALUES (@f, @u)`)
    return { curtiu: true }
  }
}
